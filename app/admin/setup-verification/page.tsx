...
export default function SetupVerificationPage() {
  ...
  const runVerification = async () => {
    ...
    try {
      ...
      let projectIdIssue;
      if (projectData.projectIdFromEnv && projectData.projectIdFromEnv.includes('"')) {
        projectIdIssue = "Project ID has quotes around it in environment variable. Remove quotes from 'project_id' in Vercel.";
      }
  ...
  const runTestImport = async () => {
    ...
    try {
      ...
      setTestImportResult({ error: error instanceof Error ? error.message : 'Unknown error' });
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <h1 className="text-3xl font-bold text-center mb-8">Setup Verification</h1>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-lg">{results.recipeCount || 0}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              {results.projectInfo.projectIdIssue && (
                <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-md">
                  <p className="text-red-800 text-sm">{results.projectInfo.projectIdIssue}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {(results.firestore.includes('❌') || results.error) && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-4 text-yellow-900">Action Required</h2>
              <div className="space-y-3">
                {results.firestore.includes('NOT_FOUND') && (
                  <div>
                    <h3 className="font-medium text-yellow-900 mb-2">Enable Firestore API:</h3>
                ...
                {results.firestore.includes('PERMISSION_DENIED') && (
                  <div>
                    <h3 className="font-medium text-yellow-900 mb-2">Enable Firebase Admin SDK API:</h3>
                    <ol className="list-decimal list-inside space-y-1 text-sm text-yellow-800">
                      <li>
                        <a
          ...
          {results.firestore.includes('✅') && (
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="space-y-4">
                <p className="text-gray-600">Test importing a single recipe to verify the system is working correctly.</p>
                <button
                ...
                {testImportResult && (
                  <div className={`p-4 rounded-md ${testImportResult.error ? 'bg-red-50 border border-red-200' : 'bg-green-50 border border-green-200'}`}>
          ...
          {/* Success Message */}
          {results.firebaseAdmin.includes('✅') && results.firestore.includes('✅') && results.spoonacular.includes('✅') && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-6">
              <h2 className="text-xl font-semibold mb-2 text-green-900">✅ Setup Complete!</h2>
              <p className="text-green-800">
                All services are properly configured. You can now{' '}
                <a href="/admin/import-recipes" className="text-blue-600 hover:underline">
