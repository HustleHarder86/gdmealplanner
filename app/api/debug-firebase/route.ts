...
export async function GET() {
  ...
  const diagnostics = {
    privateKey: {
      ...
      lastChars: privateKey
        .substring(privateKey.length - 10)
  ...
  if (
    !diagnostics.privateKey.hasNewlines &&
    diagnostics.privateKey.hasEscapedNewlines
  ) {
  ...
  if (!diagnostics.privateKey.endsWithEnd) {
    diagnostics.recommendations.push(
      "Private key should end with '-----END PRIVATE KEY-----'",
    );
  }

  if (diagnostics.privateKey.lineCount < 20) {
    diagnostics.recommendations.push(
      "Private key seems too short. It should have many lines (usually 25-30).",
    );
  }
