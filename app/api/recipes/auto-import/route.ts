...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (pauseData.paused) {
        return NextResponse.json({
          success: false,
          message:
            "Recipe imports are currently paused. Use DELETE /api/recipes/pause-import to resume.",
          paused: true,
        });
      }
    } catch (error) {
    ...
    console.log(
      `Need ${recipesNeeded} more recipes, ${recipesPerCategory} per category`,
    );
    ...
    const importBatch = async (
      ...
      try {
        ...
        console.log(
          `Imported ${result.import.imported} ${category} recipes (query: ${result.import.query})`,
        );

        return result;
      } catch (error) {
        console.error(
          `Failed to import ${category} query ${queryIndex}:`,
          error,
        );
    ...
    for (const category of ["breakfast", "lunch", "dinner", "snack"]) {
      ...
      const recipesForCategory = Math.min(
        recipesPerCategory,
        queries.length * batchSize,
      );

      for (
        let i = 0;
        i < queries.length && totalImported < recipesNeeded;
        i++
      ) {
    ...
    const processQueue = async () => {
      ...
      for (let i = 0; i < importQueue.length; i++) {
        ...
        if (running.length >= maxConcurrent || i === importQueue.length - 1) {
          ...
          console.log(
            `Processed batch, total imported so far: ${totalImported}`,
          );
