...
export async function POST() {
  try {
    ...
    const spoonacularRecipeIds = allRecipeIds.filter((id) =>
      id.startsWith("spoonacular-"),
    );

    // Identify AI-generated recipes (don't start with 'spoonacular-')
    const aiGeneratedRecipeIds = allRecipeIds.filter(
      (id) => !id.startsWith("spoonacular-"),
    );

    console.log(`Found ${spoonacularRecipeIds.length} Spoonacular recipes`);
    console.log(
      `Found ${aiGeneratedRecipeIds.length} AI-generated recipes to delete`,
    );
    ...
    for (let i = 0; i < aiGeneratedRecipeIds.length; i += batchSize) {
      ...
      console.log(
        `Deleted batch of ${batchIds.length} recipes (${totalDeleted}/${aiGeneratedRecipeIds.length} total)`,
      );
    }
    ...
    const finalSpoonacularCount = finalSnapshot.docs.filter((doc) =>
      doc.id.startsWith("spoonacular-"),
    ).length;

    console.log(
      `Cleanup complete. Final count: ${finalCount} recipes (${finalSpoonacularCount} Spoonacular)`,
    );
