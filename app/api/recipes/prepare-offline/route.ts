  ...
  try {
    ...
    if (snapshot.empty) {
      return NextResponse.json({
        success: false,
        message:
          "No recipes with local images found. Please download images first.",
      });
    }
    ...
    const offlineData = {
      ...
      metadata: {
        ...
        averageCarbs: Math.round(
          recipes.reduce((sum, r) => sum + r.nutrition.carbohydrates, 0) /
            recipes.length,
        ),
        quickRecipes: recipes.filter((r) => r.totalTime <= 30).length,
        bedtimeSnacks: recipes.filter(
          (r) =>
            r.category === "snack" &&
    ...
    for (const [category, categoryRecipes] of Object.entries(
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        byCategory: Object.entries(offlineData.categories).reduce(
          (acc, [cat, recipes]) => {
