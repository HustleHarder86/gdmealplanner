...
import { WeeklyMasterPlan, DayMeals, WeekMeals, WEEKLY_THEMES, GroceryItem, GroceryCategory } from './meal-plan-types'
...
export class MealPlanGenerator {
  ...
  private generateDayMeals(
    ...
    const breakfast = this.selectRecipe('breakfast', usedRecipes.breakfast, weekNumber)
    ...
    const morningSnack = this.selectSnack(usedRecipes.snacks, 'morning', weekNumber)
    const afternoonSnack = this.selectSnack(usedRecipes.snacks, 'afternoon', weekNumber)
    const eveningSnack = this.selectBedtimeSnack(usedRecipes.snacks, weekNumber)
  ...
  private selectRecipe(
    ...
    const available = this.allRecipes[category].filter(r => !usedSet.has(r.id))
  ...
  private selectSnack(
    ...
    const filtered = available.filter(snack => {
      if (timeOfDay === 'morning') {
        ...
        return snack.tags.some(tag => 
          ['savory', 'nuts', 'cheese', 'vegetables'].includes(tag.toLowerCase())
        )
      }
    })
    ...
    switch (theme.theme) {
      case "Mediterranean Inspired":
        return recipes.filter(r => 
          r.tags.some(tag => ['mediterranean', 'greek', 'italian'].includes(tag.toLowerCase()))
        )
      case "Quick & Easy":
        return recipes.filter(r => r.totalTime <= 30)
      case "Asian Fusion":
        return recipes.filter(r => 
          r.tags.some(tag => ['asian', 'chinese', 'thai', 'japanese'].includes(tag.toLowerCase()))
        )
      case "Budget Conscious":
        return recipes.filter(r => 
          r.tags.some(tag => ['budget-friendly', 'economical'].includes(tag.toLowerCase()))
        )
    ...
    Object.values(weekMeals).forEach(day => {
      Object.values(day).forEach((recipeId) => {
        ...
        if (recipe) {
          ...
          recipe.ingredients.forEach(ing => allIngredients.add(ing.item.toLowerCase()))
        }
      })
    })
  ...
  private aggregateQuantity(existing: string, newAmount: string, unit: string): string {
  ...
  private categorizeIngredient(ingredient: string): string {
    ...
    if (['chicken', 'beef', 'pork', 'fish', 'tofu', 'eggs'].some(p => lower.includes(p))) {
      return 'Proteins'
    }
    if (['milk', 'yogurt', 'cheese', 'cream'].some(d => lower.includes(d))) {
      return 'Dairy'
    }
    if (['lettuce', 'tomato', 'carrot', 'broccoli', 'spinach'].some(v => lower.includes(v))) {
      return 'Produce'
    }
    if (['bread', 'rice', 'pasta', 'oats', 'flour'].some(g => lower.includes(g))) {
      return 'Grains'
    }
  ...
  private organizeByCategory(ingredientMap: Map<string, GroceryItem>): GroceryCategory[] {
    ...
    const categoryOrder = ['Produce', 'Proteins', 'Dairy', 'Grains', 'Fats & Oils', 'Other']
  ...
  private getSeasonForWeek(weekNumber: number): 'spring' | 'summer' | 'fall' | 'winter' | 'any' {
