...
export class MedicalComplianceService {
  ...
  static isRecipeCompliant(recipe: Recipe): boolean {
    const guidelines =
      MEDICAL_GUIDELINES.carbs[recipe.category] ||
      MEDICAL_GUIDELINES.carbs.snacks;
    return (
      recipe.nutrition.carbs >= guidelines.min &&
      recipe.nutrition.carbs <= guidelines.max
    );
  }
  ...
  static getCarbChoices(carbs: number): number {
    return (
      Math.round((carbs / MEDICAL_GUIDELINES.portions.carbChoice) * 10) / 10
    );
  }
  ...
  static validateDailyMealPlan(meals: {
    ...
    if (!meals.breakfast) {
      ...
      if (!this.isRecipeCompliant(meals.breakfast)) {
        issues.push(
          `Breakfast carbs (${meals.breakfast.nutrition.carbs}g) outside range 25-35g`,
        );
      }
    }
    ...
    if (!meals.lunch) {
      ...
      if (!this.isRecipeCompliant(meals.lunch)) {
        issues.push(
          `Lunch carbs (${meals.lunch.nutrition.carbs}g) outside range 40-50g`,
        );
      }
    }
    ...
    if (!meals.dinner) {
      ...
      if (!this.isRecipeCompliant(meals.dinner)) {
        issues.push(
          `Dinner carbs (${meals.dinner.nutrition.carbs}g) outside range 40-50g`,
        );
      }
    }
    ...
    snacks.forEach((snack, index) => {
      if (snack) {
        ...
        if (!this.isRecipeCompliant(snack)) {
          ...
          issues.push(
            `${snackName} snack carbs (${snack.nutrition.carbs}g) outside range 15-30g`,
          );
        }
      }
    });
