...
export class RecipeService {
  ...
  static getRecipesByNutrition(filters: {
    ...
    return allRecipes.filter((recipe) => {
      ...
      if (filters.minProtein && nutrition.protein < filters.minProtein)
        return false;
      if (filters.minFiber && nutrition.fiber < filters.minFiber) return false;
      if (filters.maxCalories && nutrition.calories > filters.maxCalories)
        return false;
      return true;
    });
  }
  ...
  static getRecipeStats() {
    return {
      ...
      medicallyCompliant: allRecipes.filter((r) =>
        MedicalComplianceService.isRecipeCompliant(r),
      ).length,
    };
  }
