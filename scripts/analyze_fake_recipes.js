...
recipes.forEach((recipe) => {
  ...
  if (recipe.instructions) {
    ...
    for (const pattern of genericInstructionPatterns) {
      if (
        instructionText.includes(pattern) ||
        new RegExp(pattern).test(instructionText)
      ) {
        isFake = true;
...
console.log(
  `- Generic Instructions: ${analysisResults.patterns.genericInstructions}`,
);
...
const reportPath = path.join(
  __dirname,
  "../data/recipes/fake_recipe_analysis.json",
);
...
console.log("\nRECOMMENDATIONS:");
console.log(
  "1. Remove URL field from all fake recipes to avoid misleading users",
);
...
console.log(
  "3. Update recipe instructions with more specific, realistic steps",
);
console.log(
  "4. Consider partnering with diabetesfoodhub.org for official recipe access",
);
