...
async function checkRecipeLibrary() {
  ...
  try {
    ...
    if (totalCount > 0) {
      ...
      console.log(`   Breakfast: ${((categoryBreakdown.breakfast || 0) / totalCount * 100).toFixed(1)}%`);
      console.log(`   Lunch: ${((categoryBreakdown.lunch || 0) / totalCount * 100).toFixed(1)}%`);
      console.log(`   Dinner: ${((categoryBreakdown.dinner || 0) / totalCount * 100).toFixed(1)}%`);
      console.log(`   Snack: ${((categoryBreakdown.snack || 0) / totalCount * 100).toFixed(1)}%`);
    }
    ...
    console.log(`   Current Progress: ${((totalCount / totalOptimal) * 100).toFixed(1)}% complete`);
    ...
    for (const [category, target] of Object.entries(optimalSizes)) {
      ...
      console.log(`   ${status} ${category}: ${current}/${target} (${percentage}%)`);
    }
    ...
    console.log(`   Days remaining: ${daysRemaining} (at ${dailyImportRate} recipes/day)`);
    console.log(`   Completion date: ${new Date(Date.now() + (daysRemaining * 24 * 60 * 60 * 1000)).toLocaleDateString()}`);
