...
const verificationPath = path.join(
  __dirname,
  "../data/recipes/verification_report.json",
);
...
const cleanedRecipes = recipes.map((recipe) => {
  ...
  if (recipe.url && !validUrls.includes(recipe.url)) {
    ...
    cleanedRecipe.source_note =
      "Original recipe created for gestational diabetes meal planning";
    sourcesAdded++;
  ...
  if (!cleanedRecipe.category && cleanedRecipe.nutrition) {
    cleanedRecipe.category = determineCategoryFromNutrition(
      cleanedRecipe.nutrition,
    );
    updated = true;
  }
...
function improveInstructions(recipe) {
  ...
  if (category === "breakfast") {
    if (title.toLowerCase().includes("egg")) {
      instructions.push(
        "Crack eggs into a bowl and whisk with milk until well combined.",
      );
      instructions.push(
        "Heat a non-stick pan over medium heat and add a small amount of oil or butter.",
      );
      instructions.push(
        "Pour in egg mixture and cook, stirring gently, until eggs are just set but still creamy.",
      );
      ...
      instructions.push(
        "Simmer for 5-7 minutes, stirring occasionally, until oats are creamy.",
      );
    ...
    if (title.toLowerCase().includes("chicken")) {
      instructions.push(
        "Pat chicken dry and season both sides with salt, pepper, and your choice of herbs.",
      );
      instructions.push(
      ...
      instructions.push(
        "Serve chicken with vegetables and whole grains if using.",
      );
...
const summary = {
  ...
  recipesBySource: {
    original: cleanedRecipes.filter((r) => r.recipe_source === "original")
      .length,
...
const summaryPath = path.join(
  __dirname,
  "../data/recipes/cleaning_summary.json",
);
...
console.log("\nRecipes by source:");
console.log(
  `- Original (from diabetesfoodhub.org): ${summary.recipesBySource.original}`,
);
