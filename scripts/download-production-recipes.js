  ...
  try {
    ...
    const summary = {
      ...
      byCategory: Object.fromEntries(
        Object.entries(recipesByCategory).map(([cat, recipes]) => [cat, recipes.length])
      ),
    ...
    // Nutrition analysis
    console.log('\n=== Nutrition Analysis ===');
    const recipesWithNutrition = data.recipes.filter(r => r.nutrition && r.nutrition.carbohydrates);
    console.log(`Recipes with nutrition data: ${recipesWithNutrition.length}/${data.count}`);
    
    if (recipesWithNutrition.length > 0) {
      const carbValues = recipesWithNutrition.map(r => parseFloat(r.nutrition.carbohydrates));
