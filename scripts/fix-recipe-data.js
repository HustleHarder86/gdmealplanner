  ...
  for (const ingredient of recipe.ingredients) {
    ...
    if (dishToIngredients[lowerName]) {
      ...
      if (dishAdditionalIngredients[lowerName]) {
        fixedIngredients.push(
          ...dishAdditionalIngredients[lowerName].map((ing) => ({
            amount: ing.amount,
            unit: ing.unit,
            item: ing.name,
          })),
  ...
  return instructions.map((instruction) => {
    ...
    if (instruction.includes("according to package directions")) {
      if (recipeName.toLowerCase().includes("wrap")) {
        ...
        recipeName.toLowerCase().includes("tuna")
      ) {
...
function processRecipeFile(filePath) {
  ...
  const fixedRecipes = data.map((recipe) => {
    ...
    // Fix ingredients
    const { ingredients, modified: ingredientsModified } =
      fixRecipeIngredients(recipe);
    if (ingredientsModified) {
      recipe.ingredients = ingredients;
      modified = true;

      // Also fix the instructions since we changed ingredients
      recipe.instructions = fixRecipeInstructions(
        recipe.instructions,
        recipe.title,
      );
    }
...
function main() {
  ...
  if (totalModified > 0) {
    ...
    console.log(
      '1. Run "node scripts/generate-master-meal-plans.js" to regenerate meal plans',
    );
    console.log(
      "2. Verify the grocery lists no longer contain complete dishes",
    );
