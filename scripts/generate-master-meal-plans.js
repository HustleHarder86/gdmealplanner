...
const WEEKLY_THEMES = [
  { week: 1, theme: "Mediterranean Inspired", description: "Fresh flavors from the Mediterranean with olive oil, whole grains, and lean proteins" },
  { week: 2, theme: "Quick & Easy", description: "All meals under 30 minutes - perfect for busy weeks" },
  { week: 3, theme: "Comfort Classics", description: "Healthified versions of your favorite comfort foods" },
  { week: 4, theme: "Asian Fusion", description: "Flavorful Asian-inspired dishes with balanced nutrition" },
  { week: 5, theme: "Farm Fresh", description: "Seasonal vegetables and wholesome ingredients take center stage" },
  { week: 6, theme: "Global Flavors", description: "A culinary journey with dishes from around the world" },
  { week: 7, theme: "Summer Lighter", description: "Light, refreshing meals perfect for warmer weather" },
  { week: 8, theme: "Hearty & Wholesome", description: "Satisfying meals that keep you full and energized" },
  { week: 9, theme: "30-Minute Meals", description: "Complete meals from start to finish in 30 minutes or less" },
  { week: 10, theme: "Batch Cooking Friendly", description: "Meals that prep well for efficient weekly cooking" },
  { week: 11, theme: "Budget Conscious", description: "Delicious meals that won't break the bank" },
  { week: 12, theme: "Holiday Favorites", description: "Special occasion meals that everyone will enjoy" }
];
...
class MasterPlanGenerator {
  ...
  generateAllPlans() {
    ...
    for (let week = 1; week <= 12; week++) {
      ...
      const weekPath = path.join(__dirname, '..', 'data', 'meal-plans', `week-${week}.json`);
      fs.writeFileSync(weekPath, JSON.stringify(weekPlan, null, 2));
    }
    
    // Save master file with all plans
    const masterPath = path.join(__dirname, '..', 'data', 'meal-plans', 'master-plans.json');
  ...
  selectRecipe(category, weekNumber) {
    ...
    const available = recipes.filter(r => !this.usedRecipes[category].has(r.id));
  ...
  selectSnack(timeOfDay, weekNumber) {
    ...
    if (available.length === 0) {
      console.log(`  Note: Reusing snacks for ${timeOfDay} snack in week ${weekNumber}`);
      available = snacks;
    }
  ...
  selectBedtimeSnack(weekNumber) {
    ...
    const snacks = recipeData.snacks;
    const bedtimeSnacks = snacks.filter(s => 
    ...
    if (bedtimeSnacks.length === 0) {
      ...
      const allBedtime = snacks.filter(s => 
        s.nutrition.carbs >= 14 && 
        s.nutrition.carbs <= 16 && 
        s.nutrition.protein >= 5
      );
    ...
    const recipe = bedtimeSnacks[Math.floor(Math.random() * bedtimeSnacks.length)];
    this.usedRecipes.snacks.add(recipe.id);
  ...
  categorizeIngredient(ingredient) {
    ...
    if (['chicken', 'beef', 'turkey', 'fish', 'tofu', 'eggs', 'salmon', 'tuna'].some(p => lower.includes(p))) {
      return 'Proteins';
    }
    if (['milk', 'yogurt', 'cheese', 'cream', 'butter'].some(d => lower.includes(d))) {
      return 'Dairy';
    }
    if (['tomato', 'lettuce', 'carrot', 'broccoli', 'spinach', 'pepper', 'cucumber', 'onion'].some(v => lower.includes(v))) {
      return 'Produce';
    }
    if (['bread', 'rice', 'pasta', 'oats', 'quinoa', 'flour'].some(g => lower.includes(g))) {
      return 'Grains';
    }
  ...
  printStats(plans) {
    ...
    console.log(`Total unique recipes used: ${this.usedRecipes.breakfast.size + this.usedRecipes.lunch.size + this.usedRecipes.dinner.size + this.usedRecipes.snacks.size}`);
    console.log(`Breakfast recipes used: ${this.usedRecipes.breakfast.size}/90`);
