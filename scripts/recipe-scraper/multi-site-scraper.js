...
class MultiSiteRecipeScraper {
  ...
  async scrapeRecipe(url, source) {
    try {
      ...
      if (!this.isGDAppropriate(recipe.nutrition, recipe.category)) {
        console.log(
          `  ❌ Not GD appropriate: ${recipe.nutrition.carbs}g carbs`,
        );
        return null;
      }

      console.log(
        `  ✅ Success: ${recipe.title} (${recipe.nutrition.carbs}g carbs)`,
      );
      return recipe;
    } catch (error) {
  ...
  determineCategory(title) {
    ...
    if (/breakfast|egg|pancake|oatmeal|smoothie/.test(lower))
      return "breakfast";
...
async function main() {
  ...
  if (allRecipes.length > 0) {
    ...
    console.log(
      `Average carbs: ${Math.round(allRecipes.reduce((sum, r) => sum + r.nutrition.carbs, 0) / allRecipes.length)}g`,
    );
    console.log(
      `Average protein: ${Math.round(allRecipes.reduce((sum, r) => sum + r.nutrition.protein, 0) / allRecipes.length)}g`,
    );
    ...
    allRecipes.slice(0, 5).forEach((recipe) => {
      ...
      console.log(
        `  ${recipe.nutrition.carbs}g carbs, ${recipe.nutrition.protein}g protein`,
      );
