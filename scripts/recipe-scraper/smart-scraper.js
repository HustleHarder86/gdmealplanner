...
class SmartGDRecipeScraper {
  ...
  parseRecipeData(data, url) {
    ...
    recipe.totalTime =
      this.parseDuration(data.totalTime) || recipe.prepTime + recipe.cookTime;

    // Skip if over 45 minutes
    if (recipe.totalTime > 45) {
      console.log(
        `Skipping ${recipe.title} - too long (${recipe.totalTime} min)`,
      );
      return null;
    }
...
async function main() {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    if (recipe) {
      ...
      console.log(
        `  Carbs: ${recipe.nutrition.carbs}g, Protein: ${recipe.nutrition.protein}g`,
      );
    }
  ...
  if (successfulRecipes.length > 0) {
    ...
    successfulRecipes.forEach((recipe) => {
      ...
      console.log(
        `  Nutrition: ${recipe.nutrition.carbs}g carbs, ${recipe.nutrition.protein}g protein`,
      );
    });
  }
}
