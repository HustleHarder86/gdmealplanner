...
async function fetchRecipesFromProduction() {
  ...
  for (const baseUrl of productionUrls) {
      ...
      if (response.ok) {
        ...
        if (data.recipes && data.recipes.length > 0) {
          ...
          data.recipes.slice(0, 3).forEach((recipe, index) => {
            ...
            console.log(`   Carbs: ${recipe.nutrition?.carbohydrates || 'N/A'}g`);
    ...
    try {
      ...
      if (listResponse.ok) {
        ...
        if (listData.recipes && listData.recipes.length > 0) {
          ...
          const recipeResponse = await fetch(`${baseUrl}/api/recipes/${firstRecipeId}`);
          if (recipeResponse.ok) {
            const recipeData = await recipeResponse.json();
            console.log(`✅ Success! Retrieved: ${recipeData.title}`);
            console.log(`Full nutrition info available: ${recipeData.nutrition ? 'Yes' : 'No'}`);
            console.log(`Ingredients count: ${recipeData.ingredients?.length || 0}`);
            console.log(`Instructions count: ${recipeData.instructions?.length || 0}`);
          } else {
            console.log(`❌ Failed to fetch individual recipe! Status: ${recipeResponse.status}`);
          }
        }
      }
    } catch (error) {
    ...
    try {
      ...
      const exportResponse = await fetch(`${baseUrl}/api/recipes/export?format=json`);
      ...
      if (exportResponse.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          if (exportData.recipes) {
            console.log(`Total recipes available for export: ${exportData.recipeCount}`);
            console.log(`Export contains full recipe data: Yes`);
          } else {
            console.log(`Export summary: ${JSON.stringify(exportData.summary)}`);
          }
        }
      } else {
  ...
  console.log('- GET /api/recipes/export?format=json - Export all recipes as JSON file');
