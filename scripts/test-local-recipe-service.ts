...
async function runTests() {
  ...
  test('Get all recipes', () => {
    const recipes = LocalRecipeService.getAllRecipes();
    assert(recipes.length === 3, `Expected 3 recipes, got ${recipes.length}`);
    assert(recipes[0].title === 'Scrambled Eggs with Spinach', 'First recipe title mismatch');
  });
  ...
  test('Get recipe by ID', () => {
    ...
    assert(recipe!.title === 'Apple Slices with Almond Butter', 'Recipe title mismatch');
  });
  ...
  test('Get recipes by category', () => {
    ...
    assert(breakfast.length === 1, `Expected 1 breakfast recipe, got ${breakfast.length}`);
    
    const snacks = LocalRecipeService.getRecipesByCategory('snack');
    assert(snacks.length === 1, `Expected 1 snack recipe, got ${snacks.length}`);
  });

  // Test search
  test('Search recipes by title', () => {
    const results = LocalRecipeService.searchRecipes('eggs');
    assert(results.length === 1, `Expected 1 result for "eggs", got ${results.length}`);
    assert(results[0].title.includes('Eggs'), 'Search result should contain "Eggs"');
  });

  test('Search recipes by ingredient', () => {
    const results = LocalRecipeService.searchRecipes('chicken');
    assert(results.length === 1, `Expected 1 result for "chicken", got ${results.length}`);
  });

  test('Search recipes by tag', () => {
    const results = LocalRecipeService.searchRecipes('bedtime');
    assert(results.length === 1, `Expected 1 result for "bedtime", got ${results.length}`);
  });

  test('Search is case insensitive', () => {
    const results = LocalRecipeService.searchRecipes('APPLE');
    assert(results.length === 1, `Expected 1 result for "APPLE", got ${results.length}`);
  });

  // Test nutrition filtering
  test('Filter by max carbs', () => {
    const results = LocalRecipeService.getRecipesByNutrition({ maxCarbs: 30 });
    assert(results.length === 2, `Expected 2 low-carb recipes, got ${results.length}`);
  });

  test('Filter by min protein', () => {
    const results = LocalRecipeService.getRecipesByNutrition({ minProtein: 20 });
    assert(results.length === 1, `Expected 1 high-protein recipe, got ${results.length}`);
  });

  // Test bedtime snacks
  test('Get bedtime snacks', () => {
    const bedtimeSnacks = LocalRecipeService.getBedtimeSnacks();
    assert(bedtimeSnacks.length === 1, `Expected 1 bedtime snack, got ${bedtimeSnacks.length}`);
    const snack = bedtimeSnacks[0];
    assert(
      snack.nutrition.carbohydrates >= 14 && snack.nutrition.carbohydrates <= 16,
      'Bedtime snack carbs should be 14-16g'
    );
    assert(snack.nutrition.protein >= 5, 'Bedtime snack should have at least 5g protein');
  });

  // Test quick recipes
  test('Get quick recipes (default 30 min)', () => {
    const quickRecipes = LocalRecipeService.getQuickRecipes();
    assert(quickRecipes.length === 2, `Expected 2 quick recipes, got ${quickRecipes.length}`);
  });

  test('Get very quick recipes (10 min)', () => {
    const veryQuick = LocalRecipeService.getQuickRecipes(10);
    assert(veryQuick.length === 1, `Expected 1 very quick recipe, got ${veryQuick.length}`);
  });

  // Test random recipes
  test('Get random recipes', () => {
    const random = LocalRecipeService.getRandomRecipes(2);
    assert(random.length === 2, `Expected 2 random recipes, got ${random.length}`);
  });
  ...
  test('Get statistics', () => {
    ...
    assert(stats.withLocalImages === 2, `Expected 2 recipes with images, got ${stats.withLocalImages}`);
    assert(stats.quickRecipes === 2, `Expected 2 quick recipes, got ${stats.quickRecipes}`);
    assert(stats.bedtimeSnacks === 1, `Expected 1 bedtime snack, got ${stats.bedtimeSnacks}`);
  });
