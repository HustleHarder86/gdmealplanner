...
async function testOfflineSystem() {
  ...
  try {
    ...
    if (recipes.length === 0) {
      ...
      logTest('Initialize with empty data', 'fail', `Expected 0 recipes, got ${recipes.length}`);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (searchResults.length === 1) {
      ...
      logTest('Search functionality', 'fail', `Expected 1 result, got ${searchResults.length}`);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (breakfastRecipes.length === 1) {
      ...
      logTest('Category filtering', 'fail', `Expected 1 breakfast recipe, got ${breakfastRecipes.length}`);
    }
  } catch (error: any) {
  ...
  const recipesOfflinePath = path.join(process.cwd(), 'app/recipes-offline/page.tsx');
  ...
  const backupApiPath = path.join(process.cwd(), 'app/api/recipes/backup-all/route.ts');
  ...
  const exportApiPath = path.join(process.cwd(), 'app/api/recipes/export/route.ts');
  ...
  const prepareOfflineApiPath = path.join(process.cwd(), 'app/api/recipes/prepare-offline/route.ts');
  ...
  const componentGuidePath = path.join(process.cwd(), 'COMPONENT_MIGRATION_GUIDE.md');
  ...
  console.log('\n📝 Next Steps:');
  if (testResults.failed === 0) {
    console.log('✨ All integration tests passed! The offline system is ready to use.');
