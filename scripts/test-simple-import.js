...
async function testSimpleImport() {
  ...
  try {
    ...
    if (checkResponse.status === 404) {
      console.log('❌ Endpoint not deployed yet. Waiting for Vercel deployment...');
      return;
    }
    
    const checkData = await checkResponse.json();
    console.log('✅ Endpoint available!');
    console.log('Available queries:', JSON.stringify(checkData.availableQueries, null, 2));
  } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.import.rejectedDetails && data.import.rejectedDetails.length > 0) {
        console.log('\nRejected recipes:');
