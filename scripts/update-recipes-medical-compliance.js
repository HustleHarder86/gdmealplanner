...
function adjustCarbs(recipe, category) {
  ...
  if (currentCarbs < guideline.min) {
    ...
    } else if ((category === 'lunch' || category === 'dinner') && currentCarbs < 40) {
  ...
  const updatedRecipe = {
    ...
    ingredients: recipe.ingredients.map(ing => {
      ...
      const carbIngredients = ['bread', 'rice', 'pasta', 'potato', 'oats', 'cereal', 'fruit', 'berries', 'banana'];
      const shouldAdjust = carbIngredients.some(carb => ing.item.toLowerCase().includes(carb));
...
categories.forEach(category => {
  ...
  const updatedRecipes = recipes.map(recipe => {
    ...
    if (updated.medicallyCompliant && original !== updated.nutrition.carbs) {
      adjustedCount++;
      console.log(`${category}: "${recipe.title}" adjusted from ${original}g to ${updated.nutrition.carbs}g carbs`);
    }
...
const allRecipes = [];
categories.forEach(category => {
  const recipes = JSON.parse(fs.readFileSync(path.join(dataPath, `${category}.json`), 'utf8'));
  allRecipes.push(...recipes);
});

fs.writeFileSync(path.join(dataPath, 'recipes.json'), JSON.stringify(allRecipes, null, 2));
...
categories.forEach(category => {
  ...
  const compliant = recipes.filter(r => 
    r.nutrition.carbs >= guideline.min && 
    r.nutrition.carbs <= guideline.max
  );
  ...
  report.compliance[category] = {
    ...
    averageCarbs: Math.round(recipes.reduce((sum, r) => sum + r.nutrition.carbs, 0) / recipes.length)
  };
});

fs.writeFileSync(path.join(dataPath, 'compliance-report.json'), JSON.stringify(report, null, 2));
