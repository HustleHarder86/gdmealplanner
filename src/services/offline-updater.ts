  ...
  static async updateOfflineRecipes(): Promise<UpdateResult> {
    ...
    try {
      ...
      const metadata = {
        ...
        compressionRatio:
          ((compressed.length / (await fs.stat(fullPath)).size) * 100).toFixed(
            2,
          ) + "%",
      };
  ...
  static async validateOfflineFiles(): Promise<{
    ...
    try {
      ...
      if (data.recipeCount !== data.recipes.length) {
        issues.push(
          `Recipe count mismatch: expected ${data.recipeCount}, found ${data.recipes.length}`,
        );
      }
      ...
      for (const recipe of data.recipes) {
        for (const field of requiredFields) {
          if (!(field in recipe)) {
            issues.push(
              `Recipe ${recipe.id || "unknown"} missing required field: ${field}`,
            );
            break;
          }
        }
