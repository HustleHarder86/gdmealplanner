...
async function saveLog(message: string, level: "info" | "error" | "warn" = "info") {
...
async function main() {
  try {
    ...
    await saveLog(`Campaign day ${status.currentDay}/${status.totalDays}, Phase ${status.phase}`, "info");
    
    // Check if campaign is complete
    if (status.currentDay > status.totalDays) {
      await saveLog("Campaign complete! All 20 days have been processed.", "info");
    ...
    await saveLog(`Fatal error: ${error instanceof Error ? error.message : "Unknown error"}`, "error");
    console.error("\n❌ Daily import failed!");
    console.error(error);
    process.exit(1);
  }
